...
> 5. **Lifecycle verbs.** "Draft / publish / schedule / preview" → `versions: { drafts: true }` and `access.publish`. "Keep history / audit / restore" → `versions: true`. "Sign in / accounts" → `auth: true` on a users collection.
